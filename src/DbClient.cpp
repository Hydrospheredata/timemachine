...
namespace timemachine{
    ...
    DbClient::DbClient(std::shared_ptr<Config> _cfg){
        ...
        rocksdb::Status dbStatus = rocksdb::DBCloud::Open(options, cfg->dbName, cfDescriptors, persistent_cache, 0, &handles, &db);
        ...
        if (!dbStatus.ok()) {
            spdlog::error("Unable to open db at path {0} with bucket {1}. {2}", cfg->dbName, cfg->sourceBucket, dbStatus.ToString());
